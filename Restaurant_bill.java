...
public class Restaurant_bill extends JFrame implements ActionListener
{
    Container cp;
    PrinterJob pj;
    PageFormat pf;
...
Connection cn= DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "nasim", "khan");
		...
		rs=st.executeQuery("select * from restaurant_bill_detail");
		}catch(Exception e) { }
    ...
    public void grand_total()
    {
        try
        {
            String query1="Select sum(rbid) from emp_detail where rbid=? group by rbid";
    ...
    {
       try{
        String query = "SELECT * FROM restaurant_bill_detail " + wCOND;
        st = cn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
    ...
    {
        String query = "SELECT rbid FROM restaurant_bill_detail " ;
        try{
    ...
    public Restaurant_bill()
   {
      setBounds(10,10,780,560);
       setTitle("Restaurant bill Report ");

     cp = getContentPane();
     setDefaultCloseOperation(WindowConstants.HIDE_ON_CLOSE);
     p=new JPanel();
     p.setLayout(new GridLayout(0,7));
     lblempid = new JLabel("RbId :  ",SwingConstants.RIGHT);
    ...
    public void actionPerformed(ActionEvent e)
   {
      ...
      if(cmd.equals("Show"))
      {
        wCOND = " WHERE rbid='" + cmbemp_id.getSelectedItem()+"'";
        showGridData();
      }
    ...
    void showGridData()
  {
    openRecord();
    if(rc==0)
      JOptionPane.showMessageDialog(null,"Data not Found.","Restaurant Bill Report",JOptionPane.INFORMATION_MESSAGE);
    else
    {
      ...
      remove(t);
      t=new JPanel();
      t.setBorder(BorderFactory.createTitledBorder("Restaurant Bill_report"));
  ...
  {
    Restaurant_bill rb=new Restaurant_bill();
    rb.show();
  }
